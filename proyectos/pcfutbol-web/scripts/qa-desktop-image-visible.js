...
(async () => {
  ...
  for (const vp of viewports) {
    ...
    await page.goto('http://127.0.0.1:5178/?qa=desktop-image-visible-local-20260604c', { waitUntil: 'networkidle', timeout: 60000 });
    await page.waitForTimeout(1500);
    const metrics = await page.evaluate(() => {
      const active = document.querySelector('.mode-card-full');
      const body = document.querySelector('.mode-card-full__body');
      const media = document.querySelector('.mode-card-full__media img');
