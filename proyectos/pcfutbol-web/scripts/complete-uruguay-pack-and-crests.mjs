...
const outDir = path.resolve(root, 'scraped-data', 'uruguay-transfermarkt-crests');
...
const fakeTeams = [...(allTeams.uruguayPrimera || [])];
const scrapedTeams = JSON.parse(fs.readFileSync(path.resolve(root, 'scraped-data', '2025-26', 'uruguayPrimera.json'), 'utf8'))
  .slice()
...
if (fakeTeams.length !== scrapedTeams.length) {
  throw new Error(`Uruguay team count mismatch: fake=${fakeTeams.length} real=${scrapedTeams.length}`);
}
...
for (let i = 0; i < fakeTeams.length; i++) {
  ...
  console.log(`OK uruguay ${fakeTeam.id} -> ${realName}`);
}
...
console.log(JSON.stringify({ ok: true, uploaded, patchedNames, mapping }, null, 2));
