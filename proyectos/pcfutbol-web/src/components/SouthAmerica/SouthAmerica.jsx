...
export default function SouthAmerica() {
  ...
  const saState = state.saCompetitions;
  
  // Find player's competition
  const playerComp = useMemo(() => {
    if (!saState?.competitions) return null;
    return getPlayerSACompetition(saState, state.teamId);
  }, [saState, state.teamId]);

  // Default to player's competition, or first available
  const activeCompId = selectedComp || playerComp?.competitionId || 
    (saState?.competitions ? Object.keys(saState.competitions).find(k => saState.competitions[k]) : null);
    ...
    return (
      <div className="europe sa-comps">
        <div className="europe__empty">
          ...
          <h2>Competiciones Sudamericanas</h2>
          <p>No estás clasificado para competiciones continentales esta temporada.</p>
  ...
  return (
    <div className="europe sa-comps">
      ...
      <div className="europe__tabs">
        {Object.entries(SA_COMPETITIONS).map(([compId, comp]) => {
          const compState = saState.competitions[compId];
...
/**
 * Render knockout phase matchups (same pattern as Europe.jsx)
 */
